    ...
    checkInBtn.addEventListener('click', async () => {
      if (!isCheckedIn) {
        ...
        try {
          ...
          console.warn('Error logging check-in activity:', error);
        }
  ...
  if (checkOutBtn) {
    checkOutBtn.addEventListener('click', async () => {
      if (isCheckedIn) {
        ...
        const sessionDuration = breakStartTime
...
async function startScreenRecording(): Promise<void> {
  try {
    ...
    // Create constraints for screen capture using the modern API
    const constraints: MediaStreamConstraints = {
      audio: false,
      video: {
        // Use the modern constraint format instead of deprecated mandatory
        width: { ideal: 1920 },
        height: { ideal: 1080 },
        frameRate: { ideal: 30 }
      }
